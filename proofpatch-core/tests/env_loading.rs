...
fn env_merge_from_cursor_mcp_json() {
    ...
    fs::write(
        &mcp_path,
        r#"{"mcpServers":{"proofpatch":{"url":"http://127.0.0.1:8087","env":{"OPENROUTER_API_KEY":"TEST_KEY"}}}}"#,
    )
    .unwrap();
    ...
    std::env::set_var("PROOFPATCH_MCP_JSON_PATH", &mcp_path);
    ...
    // cleanup
    std::env::remove_var("PROOFPATCH_MCP_JSON_PATH");
    std::env::remove_var("OPENROUTER_API_KEY");
}
...
fn dotenv_sibling_search_loads_key_when_repo_env_missing() {
    ...
    std::env::remove_var("OPENROUTER_API_KEY");
    std::env::remove_var("GROQ_API_KEY");
    std::env::set_var("PROOFPATCH_DOTENV_SEARCH_ROOT", td.path());
    std::env::set_var("PROOFPATCH_DOTENV_SEARCH", "1");

    plc::load_dotenv_smart(&root);
    assert_eq!(std::env::var("OPENAI_API_KEY").unwrap(), "FROM_SIBLING");

    // cleanup for other tests
    std::env::remove_var("PROOFPATCH_DOTENV_SEARCH_ROOT");
    std::env::remove_var("PROOFPATCH_DOTENV_SEARCH");
    std::env::remove_var("OPENAI_API_KEY");
}
